  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden noise-overlay">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 py-16 lg:py-24">
        ...
        <div
          className={cn(
            "mb-12 lg:mb-16 transition-all duration-700 delay-100",
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          )}
        >
          ...
          <div className="flex flex-wrap items-center gap-4 lg:gap-0">
            {pipelineSteps.map((step, index) => (
              <div key={step.id} className="flex items-center">
                <div
                  className={cn(
                    "flex items-center gap-3 px-5 py-3 rounded-xl border transition-all duration-300 hover-lift",
                    activeStep === index
                ...
                {index < pipelineSteps.length - 1 && (
                  <ArrowRight className="w-4 h-4 mx-2 text-foreground/20 hidden lg:block flex-shrink-0" />
                )}
              </div>
            ))}
          </div>
        ...
        <p
          className={cn(
            "mt-12 text-sm text-muted-foreground max-w-2xl transition-all duration-700 delay-300",
